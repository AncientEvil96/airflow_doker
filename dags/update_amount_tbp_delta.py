...
project_name = 'update_amount_tbp_delta'
...
def update_amount_tbp():
    ...
    e_amount = DockerOperator(
        task_id='e_amount',
        image=image,
        container_name='e_amount{{ task_instance.job_id }}',
        api_version='auto',
        auto_remove=True,
        environment={
            'MS': ms_s
        ...
        command=f'bash -c "python project/e_amount.py $MS"',
    ...
    tar_file = 'amount.tar.gz'

    tar_data = BashOperator(
        task_id='tar_data',
        bash_command=f'bash -c "cd {airflow_work_dir}/ && tar -czvf {tar_file} amount.csv"',
        trigger_rule='all_success'
    )
    ...
    create_folder >> e_amount
    e_amount >> t_amount >> tar_data
