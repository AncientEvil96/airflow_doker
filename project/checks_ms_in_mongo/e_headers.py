...
def extract_headers(sourse):
    ...
    SELECT substring(sys.fn_sqlvarbasetostr([_Document16].[_IDRRef]), 3, 32)                               as uuid_db
         ...
         , CASE
               WHEN [_Reference73]._Description = '999-999-99-99' THEN 0
               ELSE CONVERT(bigint, REPLACE([_Reference73]._Description, '-', '')) END                     as phone
    ...
    FROM [_Document16]
             LEFT JOIN [_Reference37] AS [_Reference37] ON [_Document16].[_Fld19RRef] = [_Reference37].[_IDRRef]
             INNER JOIN [_Reference73] AS [_Reference73] ON [_Document16].[_Fld117RRef] = [_Reference73].[_IDRRef]
    WHERE [_Posted] = 1
